...
const chatroom = () => {
  ...
  //func to see connection to the 8000 port for socket server
  ...
  const sendMessage = async (senderId, receiverId) => {
    ...
    //Call the fetch messages functions to see the UI update
    setTimeout(() => {
      fetchMessages();
    }, 200);
  };
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: "white" }}>
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        {messages.map((item, index) => (
          ...
          >
            <Text style={{fontSize:13,textAlign:"left",color:"white",fontWeight:"500"}}>{item?.message}</Text>
